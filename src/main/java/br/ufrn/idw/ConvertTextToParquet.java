...
public class ConvertTextToParquet {
  ...
  public static void main(String[] args) {
    ...
    SparkSession spark =
        SparkSession.builder().appName("Text to parquet").master("local[*]").getOrCreate();
    ...
    Dataset<Row> df = spark.read().format("csv").option("header", "false").option("sep", ",")
        .schema(schema).load("data/measurements.txt");
